    ...
    <div class="container">
        ...
        <form action="{{ route('admin.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="jshshr" id="jshshr" class="form-control" maxlength="14"
                    value="{{ old('jshshr') }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="passport_id" id="passport_id" class="form-control"
                    value="{{ old('passport_id') }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="student_phone" id="student_phone" class="form-control"
                    value="{{ old('student_phone') }}" required>
            </div>

            <div class="mb-3">
                <label for="parents_phone" class="form-label">Ota-ona telefon raqami</label>
                <input type="text" name="parents_phone" id="parents_phone" class="form-control"
                    value="{{ old('parents_phone') }}" required>
            </div>
            <div class="mb-3">
                ...
                <select name="address_type" id="address_type" class="form-select" required>
                    <option value="">Tanlang</option>
                    <option value="Yotoqxona" {{ old('address_type') == 'Yotoqxona' ? 'selected' : '' }}>Yotoqxona</option>
                    <option value="Ijara" {{ old('address_type') == 'Ijara' ? 'selected' : '' }}>Ijara</option>
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="housing_type" id="housing_type" class="form-select" required>
                    <option value="">Tanlang</option>
                    <option value="dormitory" {{ old('housing_type') == 'dormitory' ? 'selected' : '' }}>Yotoqxona</option>
                    <option value="rental" {{ old('housing_type') == 'rental' ? 'selected' : '' }}>Ijara uy</option>
                </select>
            </div>

            <div class="mb-3">
                <label for="address" class="form-label">Manzil | Xona raqami</label>
